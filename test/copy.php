...
$myFile = "/mnt/fs4_encoding/04-for-qc/test/".$name;
...
$ssh = new Net_SSH2('glen-mover-1.mvfinc.com');
...
if(isset($_POST['submit'])) {
	...
	$folder=$title."-".$year;
	echo "+ ".$folder."<br>";
	$directory="/mnt/fs4_encoding/04-for-qc/test/".$folder;
		...
		if(!empty($files[$i][0])) {
			...
			if($ext[1]=="m2t") {
				...
				if(!empty($files[$i][1])) {
					...
					$ep_dir="/mnt/fs4_encoding/04-for-qc/test/".$folder."/".$e_dir;
...
fclose($fh);
$cmd="php /home/kevin/test.php ".$name." > /dev/null &";
$ssh->exec($cmd);
